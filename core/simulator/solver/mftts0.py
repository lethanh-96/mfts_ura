...
class MeanFieldTransferThompsonSampling0Solver(ThompsonSamplingSolver):
    ...
    def initialize_state(self):
        ...
        for k in range(K_tot):
            ...
            self.mean[k, :] = torch.max(mean[-100:, indices, :], dim=0).values
            self.c[k, :]    = torch.max(c[:, indices, :], dim=0).values
